...
export const ORGAN_DBS = Object.freeze({
  // paper entries exist ONLY where the paper organ's work-unit verified compatible (2026-08-25):
  // claims=cases, legal=matters (DB naming diverges AGAIN: fallclaimpaper-v1 but falllegalpaper-db).
  // mortgage (cases) + insurance (policies) HAVE work-units but their lifecycle vocab ('dip-obtained',
  // 'in-force') and UI dispatch are unverified — no entry until verified, honest 0 over invention.
  // The other six paper organs are client-only (no work-unit store at all) — nothing to signal from.
